...
async function main() {
  ...
  for await (const map of maps) {
    ...
    for await (const agent of team1Agents) {
      const agentRandomRank =
        randomRanks[Math.floor(Math.random() * randomRanks.length)].id;
      console.log(agentRandomRank);
      console.log(agent.id);
      console.log(blueTeam.id);
...
function getRandomAgent(agents, teamAgents) {
  const remainingAgents = agents.filter(
    (a) => !teamAgents.map((t) => t.id).includes(a.id)
  );
